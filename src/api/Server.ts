...
export class GraphQLServer {
    ...
    constructor(props: Props) {
        ...
        if (props.schema) {
            ...
            const uploadMixin = typeDefsString.includes('scalar Upload') ? { Upload: GraphQLUpload } : {};
        ...
        if (props.middlewares) {
            const { schema, fragmentReplacements } = applyFieldMiddleware(this.executableSchema, ...props.middlewares);
    ...
    createHttpServer(options: Options): HttpServer | HttpsServer {
        ...
        const formatResponse = (req: express.Request) => {
            ...
            return (response, ...args) => {
                ...
                return this.options.formatResponse ? this.options.formatResponse(response, ...args) : response;
            };
        };
        ...
        if (this.options.uploads) {
            app.post(this.options.endpoint, graphqlUploadExpress(this.options.uploads));
        ...
        const server = this.options.https ? createHttpsServer(this.options.https, app) : createServer(app);
    ...
    start(
        ...
        return new Promise((resolve, reject) => {
            ...
            const port = typeof this.options.port !== 'number' ? parseInt(this.options.port) : this.options.port;
